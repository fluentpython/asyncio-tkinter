...
class GuiEventLoop(BaseEventLoop):
    ...
    def getaddrinfo(self, host, port, *,
        ...
        return self._io_helper(self._io_event_loop.getaddrinfo,
                               (host, port),
    ...
    def getnameinfo(self, sockaddr, flags=0):
        return self._io_helper(self._io_event_loop.getnameinfo,
                               (sockaddr, flags),
                               {}
                              )
    ...
    def create_connection(self, protocol_factory, host=None, port=None, *,
        ...
        return self._io_helper(self._io_event_loop.create_connection,
    ...
    def start_serving(self, protocol_factory, host=None, port=None, *,
                      family=0, proto=0, flags=0, sock=None):
        return self._io_helper(self._io_event_loop.start_serving,
                               (protocol_factory, host, port),
                               {'family': family, 'proto': proto,
                                'flags': flags, 'sock': sock}
                              )
    ...
    def sock_recv(self, sock, nbytes):
        return self._io_helper(self._io_event_loop.sock_recv,
                               (sock, nbytes),
                               {}
                              )

    def sock_sendall(self, sock, data):
        return self._io_helper(self._io_event_loop.sock_sendall,
                               (sock, data),
                               {}
                              )

    def sock_connect(self, sock, address):
        return self._io_helper(self._io_event_loop.sock_connect,
                               (sock, address),
                               {}
                              )
    ...
    def sock_accept(self, sock):
        return self._io_helper(self._io_event_loop.sock_accept,
                               (sock, ),
                               {}
                              )
